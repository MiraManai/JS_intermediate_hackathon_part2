...
// console.log(arguments);  test
  ...
  let excludeList = Array.from(arguments).slice(1)
// arr =[1,2,3,1,2,3]
// excludeList =[2,3]

  let uniqueArray = arr.filter(function (el) {
    return !excludeList.includes(el)
  })
  return uniqueArray
}
...
function getIndexToIns (arr, num) {
  ...
  let indexOfNum = sortedArr.findIndex(function index (element) {
    return element >= num
  })
...
function diffArray (arrA, arrB) {
  ...
  arrC = arrB.filter(function (el) {
    return arrA.indexOf(el) < 0
  })
  arrD = arrA.filter(function (el) {
...
function confirmEnding (str, target) {
  ...
  var res = str.substring(str.length - lenTarget, str.length)
